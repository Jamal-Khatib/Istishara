...
class UserController extends GetxController {
  ...
  Future<void> setProfile(String about, String skill1, String skill2,
      String skill3, String skill4, String skill5, String skill6) async {
    ...
    if (skill6 != myUser.value.skill6 && skill6 != null) {
      ...
      FirebaseFirestore.instance
          .collection("users")
          .doc(myUser.value.uid)
          .update({"skill6": skill6});
    }

    // if (rating != myUser.value.rating && rating != null) {
    //   myUser.update((val) {
    //     val.rating = rating;
    //   });
    //   FirebaseFirestore.instance
  ...
  void add_question({String question, String category}) {
    ...
    if (category == "architect") {
      myUser.update((value) {
        value.architect = question;
      });
    } else if (category == "civil") {
      myUser.update((value) {
        value.civil = question;
      });
    } else if (category == "construction") {
      myUser.update((value) {
        value.construction = question;
      });
    } else if (category == "doctor") {
      myUser.update((value) {
        value.doctor = question;
      });
    } else if (category == "electrical") {
      myUser.update((value) {
        value.electrical = question;
      });
    } else if (category == "family") {
      myUser.update((value) {
        value.family = question;
      });
    } else if (category == "heating") {
      myUser.update((value) {
        value.heating = question;
      });
    } else if (category == "mechanical") {
      myUser.update((value) {
        value.mechanical = question;
      });
    } else if (category == "psych") {
      myUser.update((value) {
        value.psych = question;
      });
    }
  }
  ...
  Future<bool> is_intersted({String ClientId}) async {
    var myclient = await FirebaseFirestore.instance
        .collection("users")
        .doc(ClientId)
        .get();
  ...
  Future<String> si({String ClientId}) async {
    if (await is_intersted(ClientId: ClientId))
      return ("Picked");
    else
      return ("");
  }
